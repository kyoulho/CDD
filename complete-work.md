...
- 처음에는 "Fast Path", "완료 조건", "다음 단계"만 먼저 본다.
...
완료 요청이면 먼저 verification 상태, 사용자 review 여부, 작업 기준서와 실제 변경 범위 일치 여부, `cdd-audit docs --root <project> --format brief --fail-on never` 결과만 확인한다. 모두 충족되고 남은 일이 완료 기록 정합성뿐이면 같은 요청 범위 안에서 완료 기록 정리와 다음 후보 제시까지 진행한다.
