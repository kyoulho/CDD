...
후속 작업이 바로 실행 가능한 상태가 아니라 "작업 기준서 작성 승인", "구현 지시서 초안 작성 승인", "구현 지시서 실행 승인" 같은 사용자 승인 대기 상태이고 사용자가 실제 다음 단계 진행을 요청했다면 승인 문장만 남기지 않는다. 완료 보고 뒤에 `_user-facing-language.md`의 "후속 작업 승인 요청 브리핑" 전체 형식을 붙여 사용자가 무엇을 승인하는지 먼저 검토하게 한다. "승인하면 내가 진행할 일"만 나열하는 것은 브리핑이 아니다.
