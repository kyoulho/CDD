...
- 완료 기록: 검증과 review approval 뒤 남기는 완료 기록.
...
verification과 사용자 review approval 없이는 complete하지 않는다.

Completion record artifact는 `_artifact-metadata.md`, `_artifact-templates.md`의 `Artifact Metadata Template`, `_status-machine.md`를 따른다. 완료 승인은 `_approval-reference.md`의 REVIEW_APPROVAL 또는 COMPLETION_APPROVAL과 연결한다.
...
- VERIFIED: user review approval 요청 가능. 승인 후 complete 가능.
...
- 사용자 review approval, 미확정 결정, 정책 충돌, 선행 Task 미완료가 있으면 완료하지 않고 선택지, 제 추천, 바로 답할 수 있는 문장을 제공한다.
