...
명시한 heading을 찾을 수 없으면 `cdd-audit`는 해당 항목을 `missing`으로 표시하고 `SECTION_HINT_MISSING_HEADING` warning을 낸다. 이 warning은 자동 수정 승인이 아니다. 에이전트는 heading을 추측해 읽었다고 처리하지 말고, current-work 또는 `.cdd-audit.json`의 섹션 계약을 현재 문서 구조에 맞게 고치는 후보를 보고한다.
