...
사용자가 "다음 작업이 무엇인가?", "현재 어디까지 왔는가?", "무엇이 남았는가?"처럼 상태나 다음 후보만 물으면 `STATUS_INQUIRY`로 본다. 조회형 질문은 후속 작업 승인, 구현 지시서 초안 작성 승인, 구현 실행 승인으로 승격하지 않는다.
...
사용자가 "다음 작업 진행하자", "다음 단계로 가자", "진행하자"라고 했고 현재 작업 포인터가 다음 승인 단계를 명확히 가리키면 단순 조회가 아니다. 바로 실행할 수 없고 승인 문장이 필요하다면 `_approval-briefing-language.md`의 "후속 작업 승인 요청 브리핑" 전체 형식으로 답한다. 반복된 같은 요청이어도 브리핑 필수 항목을 승인 문장만 남기는 방식으로 줄이지 않는다. "승인하면 내가 진행할 일"만 나열하고 승인 문장을 요구하지 않는다.

## 빠른 탐색

- 사용자 개입이 필요한지 먼저 보려면 "사용자 개입 필요 여부"를 본다.
- 각 작업 모드의 허용 범위는 "작업 모드"를 본다.
- 요청을 어떤 모드로 볼지 애매하면 "모드 판별 우선순위"를 본다.
- 분석에서 실제 수정으로 넘어가는 조건은 "분석에서 패치로 전환하는 조건"을 본다.
- 하네스 자체를 고칠 때는 "하네스 에이전트에 대한 적용"을 본다.
- 사용자에게 묻는 표현은 "사용자-facing 응답"을 본다.
...
- 검증 방법이 명확하다.
- 선행 작업 완료 정리가 필요한 경우에는 선행 작업 검증이 완료되었고 남은 일이 completion record, status, 실행 전 문구 정리 같은 artifact 정합성 보정뿐이다.
...
후속 작업 요청이 선행 작업 완료 기록 정합성에 막히는 경우, CDD Agent는 먼저 `_approval-reference.md`의 후속 작업 전환 연계 조건을 확인한다. 조건을 모두 만족하면 사용자에게 승인 문구를 요구하지 않고 선행 작업 완료 기록을 정리한 뒤 후속 작업으로 이어간다. 조건을 만족하지 못하면 무엇이 부족한지 자연어로 설명하고 필요한 선택만 묻는다.

## 작업 모드

### NEXT_STEP_REQUEST

사용자가 다음 작업을 실제로 진행하자고 요청한 상태다. 현재 작업 포인터나 작업 기준서가 다음 task와 다음 승인 단계를 명확히 가리키면 이 모드로 본다.
...
- 현재 작업 포인터와 작업 기준서를 읽고 다음 승인 단계가 무엇인지 확인한다.
- 사용자 개입 없이 진행 가능한 조건을 만족하면 같은 요청 범위 안에서 진행한다.
- 바로 실행할 수 없고 구현 지시서 초안 작성 같은 승인 단계가 필요하면 `_approval-briefing-language.md`의 "후속 작업 승인 요청 브리핑" 전체 형식으로 답한다.

금지 행동:

- 단순 조회 응답으로 축소
- 현재 포인터가 단일 다음 task를 가리키는데 일반 선택지 목록으로 먼저 돌림
- "선택지: 1. 승인한다 2. 우선순위를 바꾼다 3. 보류한다"만 제시하고 승인 대상 브리핑을 생략
- 승인 문장을 요구하면서 포함되는 것, 제외되는 것, 고정되는 결정, 주의할 점을 생략
- 반복 요청이라는 이유로 브리핑을 "승인 대상은 하나"와 승인 문장만으로 축약
- 사용자-facing 보고에 `PROMPT_DRAFT_APPROVAL` 같은 내부 approval enum을 그대로 노출

### STATUS_INQUIRY

사용자가 현재 상태, 다음 작업 후보, 남은 승인, 진행 조건만 묻는 상태다. 이 모드는 읽기와 판단만 허용하며, 승인 요청이나 파일 수정을 시작하는 모드가 아니다.
...
- 현재 작업 포인터, 작업 기준서, 완료/검증 기록을 읽고 현재 상태를 요약한다.
- 다음 작업 후보와 그 이유를 말한다.
- 바로 진행 가능한지, 아직 필요한 결정이나 승인 단계가 있는지 설명한다.
- 다음 작업 후보가 구현 지시서 초안 작성처럼 승인 대상 산출물이라면 목적, 포함 범위, 제외 범위, 주의할 점, 진행 후 가능해지는 것을 짧게 브리핑한다.
- 사용자가 진행을 원할 때 사용할 수 있는 문장을 "진행하려면 이렇게 말할 수 있습니다" 형식으로 제시한다.

금지 행동:

- 구현 시작
- 파일 생성, 수정, 삭제
- 작업 기준서, 구현 지시서, 검증 결과, 완료 기록 생성 또는 수정
- 승인 문장을 "아래 문장으로 승인해 주세요"처럼 필수 다음 행동으로 요구
- 후속 작업 승인 대기 상태를 이유로 조회형 질문을 `PROMPT_DRAFT_APPROVAL`, `PROMPT_EXECUTION_APPROVAL`, `PATCH_APPROVAL`, `APPLY_APPROVAL`로 승격

사용자-facing 응답 예:

```text
다음 작업은 TASK-002 구현 지시서 초안 작성입니다.
아직 바로 구현 단계는 아닙니다.

이유:
- TASK-002 작업 기준서는 승인됐지만 구현 지시서가 아직 없습니다.
- 구현 지시서가 있어야 구현 범위, 제외 범위, 검증 기준을 고정할 수 있습니다.

진행 후보 브리핑:
- 목적: 구현 범위, 금지 범위, 검증 기준을 고정합니다.
- 포함되는 것: 기준 문서와 작업 기준서 확인, 구현 범위/제외 범위 정리, 검증 명령과 완료 조건 정리
- 제외되는 것: 실제 코드 구현, 기준 문서 변경, 새 정책 결정
- 주의할 점: 초안 작성 중 기준이 비어 있으면 구현으로 넘어가지 않습니다.

진행하려면 이렇게 말할 수 있습니다:
"TASK-002 구현 지시서 초안 작성을 진행해라."
```
...
단, 현재 작업 포인터와 승인된 작업 기준서가 다음 승인 단계를 명확히 가리키고 사용자가 "다음 작업 진행하자"처럼 실제 진행 의사를 보이면, 모호한 확인 질문으로만 끝내지 않는다. 실제 실행에 필요한 승인 범위를 브리핑하고 바로 답할 수 있는 문장을 제시한다.
