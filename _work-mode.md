...
사용자가 "다음 작업 진행하자", "다음 단계로 가자", "진행하자"라고 했고 현재 작업 포인터가 다음 승인 단계를 명확히 가리키면 단순 조회가 아니다. 바로 실행할 수 없고 승인 문장이 필요하다면 `_user-facing-language.md`의 "후속 작업 승인 요청 브리핑" 전체 형식으로 답한다. "승인하면 내가 진행할 일"만 나열하고 승인 문장을 요구하지 않는다.
...
- 바로 실행할 수 없고 구현 지시서 초안 작성 같은 승인 단계가 필요하면 `_user-facing-language.md`의 "후속 작업 승인 요청 브리핑" 전체 형식으로 답한다.
