...
- 버전관리 확인: stage, commit, push, branch, PR, tag, rebase, amend, force-push 작업 전에 포함 변경, 제외 변경, 원격 대상, 히스토리 위험을 고정하는 절차.
- 버그리포트 확인: issue/report 작성 전에 대상 tracker, 재현 절차, 기대/실제 결과, 환경, 영향도, 증거, 비밀정보 제거를 고정하는 절차.
...
Plan과 작업 기준서를 만들거나 수정하기 전에는 `_source-of-truth-manager.md`의 문서 배치, 현재 작업 포인터, 기본 읽기 경로, active/history 분리 규칙을 따른다. 저장 전에는 `_artifact-templates.md`의 Document Placement Check를 작성하고, 새 파일이 필요하면 왜 기존 문서에 추가하지 않는지 보고한다.

`cdd-audit` 또는 수동 문서 정합성 확인에서 차단 항목이 나오면 작업 기준서를 APPROVED로 만들지 않는다. current pointer, 기본 읽기 경로, README/index 갱신, 비-SOT 표시 후보를 사용자-facing 언어로 먼저 보고한다.
...
Git 작업을 계획하려면 버전관리 확인 결론이 `VERSION_CONTROL_ALLOWED`여야 한다. 버그리포트 작성 또는 등록을 계획하려면 버그리포트 확인 결론이 `BUG_REPORT_READY`여야 한다.

사용자-facing 계획 보고는 `_user-facing-language.md`를 따른다. 내부 enum은 작업 기준서 YAML과 에이전트 간 전달물에만 그대로 둔다.

## 빠른 탐색

- 처음에는 "최소 읽기 경로", "시작 조건", "다음 단계"만 먼저 본다.
...
- 버전관리 확인이 없거나 `VERSION_CONTROL_BLOCKED`이면 stage, commit, push, branch, PR, tag, rebase, amend, force-push 작업을 `acceptanceCriteria`, `implementationConstraints`, `allowedScope`, `testRequirements`에 넣지 않는다.
- 버그리포트 확인이 없거나 `BUG_REPORT_BLOCKED`이면 issue 작성, 외부 tracker 등록, bug report 게시 작업을 `acceptanceCriteria`, `implementationConstraints`, `allowedScope`, `testRequirements`에 넣지 않는다.
...
- Git 작업 Task에는 `versionControlContract`를 포함한다. 이 계약은 요청된 Git 작업, 포함할 변경, 제외할 변경, stage 방식, commit grouping, 메시지 기준, push 대상, history rewrite 또는 force-push 위험, recovery path를 포함해야 한다.
- unrelated dirty work가 있거나 사용자가 만든 변경과 에이전트 변경이 섞여 있는데 포함/제외 범위가 없으면 Git 작업 Task를 APPROVED로 만들지 않는다.
- rebase, amend, force-push, protected branch push, tag 생성, PR 생성은 사용자 요청과 원격 대상이 명확하지 않으면 Task를 APPROVED로 만들지 않는다.
- 버그리포트 Task에는 `bugReportContract`를 포함한다. 이 계약은 target tracker, 제목, 실제 결과, 기대 결과, 재현 절차, 환경, 영향도, 증거, 비밀정보 제거, 원하는 후속 조치를 포함해야 한다.
- 재현 절차, 실제/기대 결과, 증거, 비밀정보 제거 기준 중 하나라도 없으면 bug report Task를 APPROVED로 만들지 않는다.
...
- 기존 스택으로 Task 범위와 검증 기준을 충족할 수 없으면 dependency 금지를 이유로 부정확한 Task를 만들지 않는다. dependency 목적, 범위, 대안, 영향, 검증 방법을 브리핑하고 사용자 승인 여부를 묻는다.
...
작업 기준서 승인 요청 전에는 `_approval-briefing-language.md`의 "승인 전 브리핑 형식"을 반드시 사용한다. 브리핑은 먼저 사용자가 확인해야 할 정책/작업 결정과 각 결정에 대한 추천을 제시해야 한다. 승인 문장은 이 승인이 허용하는 작업, 아직 허용하지 않는 작업, 승인하면 고정되는 결정, 위험/중단 조건, 승인 후 실제로 진행할 일을 브리핑한 뒤에만 제시한다. 브리핑 없이 작업 기준서 승인 문장만 출력하지 않는다.

브리핑 없이 승인 요청 금지. 몇 번째 승인 요청이든 작업 기준서 승인, 구현 지시서 초안 작성 승인, 구현 지시서 실행 승인, 검증 결과 승인, 완료 기록 승인 앞에는 사용자가 무엇을 승인하는지 판단할 수 있는 브리핑이 있어야 한다.

작업 기준서 승인과 구현 지시서 초안 작성 승인을 한 문장으로 묶는 경우에도 두 개의 승인 대상이 있다. 결합 승인 문장만 제시하지 말고, 작업 기준서 승인 브리핑과 구현 지시서 초안 작성 브리핑을 모두 먼저 출력한다. 작업 기준서 승인 브리핑은 작업 기준서가 무엇을 고정하는지, 포함/제외 범위, 승인하면 고정되는 결정, 아직 허용하지 않는 작업을 설명한다. 구현 지시서 초안 작성 브리핑은 구현 지시서를 어떻게 작성할지, 구현 지시서에 담길 주요 섹션, 각 섹션에서 고정할 판단, 구현자에게 금지할 행동, 초안 작성 중 다시 멈출 조건을 설명한다.
