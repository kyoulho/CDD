...
구현 후 검증은 작업 기준서대로 테스트와 확인이 끝났는지 판단해야 한다. 작업 기준서의 allowedScope, forbiddenScope, acceptanceCriteria, testRequirements, verificationCommands를 변경 결과와 대조하지 않은 보고는 완료 근거가 아니다.
...
Verification result를 만들거나 수정하기 전에는 `_source-of-truth-manager.md`의 문서 배치, 현재 작업 포인터, 기본 읽기 경로, active/history 분리 규칙을 따른다. 저장 전에는 `_artifact-templates.md`의 Document Placement Check를 작성하고, 새 파일이 필요하면 왜 기존 문서에 추가하지 않는지 보고한다.

검증 기록은 기본적으로 그 시점의 사실 기록이며 현재 기준 문서가 아니다. 과거 검증 기록이나 완료 기록을 현재 기준처럼 읽어야만 검증할 수 있으면 먼저 문서 정합성 finding으로 보고한다.
...
- 처음에는 "최소 읽기 경로", "입력", "최소 guard checklist", "결과 상태"만 먼저 본다.
- 검증에 필요한 입력은 "입력"과 "최소 guard checklist"를 먼저 본다.
- 검증 대상별 세부 항목은 "검증 항목"과 "정책 위반 유형"을 본다.
- 실패가 환경 문제인지 코드 문제인지 나누려면 "환경 실패와 코드 실패 구분"을 본다.
- 저장/API/status/상호작용/운영 기준 검증은 "Identifier/API/Data Model 검증", "Interaction / Operational Quality 검증"을 본다.
- 테스트 전략은 "Test Strategy 검증"을 본다.
- 결과 판정은 "결과 상태"와 각 status 예시를 본다.
- 사용자 보고는 "User-Facing Reporting"을 따른다.

## 최소 읽기 경로

검증 요청이면 먼저 구현 diff 또는 변경 파일, check result, 작업 기준서, `cdd-audit docs --root <project> --format brief --fail-on never`가 가리킨 현재 기준 문서만 읽는다. 이 경로는 검증 항목을 줄이는 것이 아니라 과거 기록을 현재 기준으로 오독하지 않게 하는 절차다. 과거 verification/completion은 current pointer가 요구하지 않으면 기본으로 열지 않는다.

검증 결과 작성, 완료 전환, source of truth 변경, revision 판단이 필요할 때만 `_artifact-metadata.md`, `_artifact-templates.md`, `_status-machine.md`, `_approval-reference.md`, `_source-of-truth-manager.md`, `_user-facing-language.md`를 연다.
...
- 구현 prompt
- check result
- verificationCommands 실행 결과 또는 실행 불가 사유
- 구현 diff 또는 변경 파일
- result/report
- 작업 기준 묶음
- 준비 상태 확인 결과
...
- task-contract verification guard: 작업 기준서의 allowedScope, forbiddenScope, acceptanceCriteria, testRequirements, verificationCommands를 변경 결과와 대조했는지 확인한다.
...
- document read-cost guard: 기본 읽기 경로의 문서가 400줄 또는 40KB를 넘거나 1000줄 이상 누적되었는데 분리 후보/유지 후보가 보고되지 않았는지 확인한다.
- active/history separation guard: 과거 task, completion, verification, prompt가 현재 기준처럼 사용되었는지 확인한다.
- non-SOT reference guard: generated map, Codesight, agentmemory, search index, recall output, archive branch reference가 active 기준처럼 사용되었는지 확인한다.
...
- frontend UX role coverage guard: 승인 문서 안의 `FRONTEND_UX_CRITERIA`, `USER_FLOW` 또는 `INTERACTION_SPEC`, `DESIGN_SYSTEM` 또는 `UI_PATTERN`, `FRONTEND_ARCHITECTURE` 역할 coverage 없이 웹/모바일 UI 기준이 제안되거나 구현되었는지 확인한다.
- UI implementation contract guard: 웹/모바일 UI 구현이 `uiImplementationContract` 없이 컴포넌트 단위 수정으로 진행되었는지 확인한다.
- screenshot intent comparison guard: 브라우저/스크린샷 결과를 UI 구현 계약의 레이아웃, 정보 우선순위, 금지 패턴, 반응형 기준과 대조했는지 확인한다.
...
- version-control contract guard: stage, commit, push, branch, PR, tag, rebase, amend, force-push가 `versionControlContract` 없이 수행되었는지 확인한다.
- git scope guard: staged/committed/pushed 변경이 includeChanges 안에 있고 excludeChanges, unrelated dirty work, 사용자 소유 변경을 포함하지 않았는지 확인한다.
- bug-report contract guard: issue 작성, 외부 tracker 등록, bug report 게시가 `bugReportContract` 없이 수행되었는지 확인한다.
- bug-report evidence guard: 버그리포트가 재현 절차, 실제/기대 결과, 환경, 영향도, 증거, redaction 확인을 포함하는지 확인한다.
...
Verification 결과에는 작업 기준서 기준으로 무엇을 확인했는지 반드시 포함한다. 테스트 명령을 실행했다는 사실만으로 충분하지 않다. 작업 기준서가 요구한 범위, 금지 범위, acceptance criteria, test requirements, verification commands를 변경 결과와 대조해야 한다.
...
필수 verificationCommands를 실행하지 못했고 작업 기준서나 승인 문서가 허용한 대체 검증도 없으면 `VERIFIED`로 판단하지 않는다. 이 경우 "아직 완료로 기록할 수 없습니다"라고 보고하고, 필요한 환경, 명령, 사용자 결정, 또는 작업 기준서 보완 지점을 제시한다.
...
- 웹/모바일 UI라면 requiredDocuments의 승인 문서 안에서 `FRONTEND_UX_CRITERIA`, `USER_FLOW` 또는 `INTERACTION_SPEC`, `DESIGN_SYSTEM` 또는 `UI_PATTERN`, `FRONTEND_ARCHITECTURE` 역할 coverage가 확인되는지 확인한다.
- 웹/모바일 UI라면 `uiImplementationContract`가 있고 구현 결과가 화면 단위로 그 계약을 만족하는지 확인한다.
- 웹/모바일 UI라면 DOM 존재 여부나 테스트 통과만 보지 말고 현재 사용자 화면 폭을 먼저 확인한 뒤 필요한 desktop/mobile viewport를 확인한다.
...
- Git 작업이 포함된 경우 버전관리 확인이 허용 결론이었고, 변경 범위와 원격 대상이 계약과 일치하는지 확인한다.
- 버그리포트 작업이 포함된 경우 버그리포트 확인이 준비 결론이었고, 재현성과 증거와 redaction 확인이 계약과 일치하는지 확인한다.
- verification matrix를 작성한다.
- source of truth와 충돌하는 구현을 찾는다.
- 문서에 없는 도메인/아키텍처/행위 판단이 코드에 들어갔는지 확인한다.
- 작업 기준서 기준 검증이 끝났는지 판단한다.
- 작업 기준서 기준 검증이 끝나지 않았으면 완료 처리로 넘기지 않는다.
- 수정이 필요한지 판단한다.
...
- verification result 저장 전 사용자 보고에 분리 후보, 유지 후보, 삭제/보존/비-SOT 분류 후보를 포함한다.
...
verificationCommands 실행 또는 승인된 대체 검증 여부
Task 범위 초과 여부
...
승인 문서 안의 FRONTEND_UX_CRITERIA, USER_FLOW 또는 INTERACTION_SPEC, DESIGN_SYSTEM 또는 UI_PATTERN, FRONTEND_ARCHITECTURE 역할 coverage 없이 웹/모바일 UI 기준을 제안하거나 구현했는지 여부
uiImplementationContract 없이 웹/모바일 UI 구현을 진행했는지 여부
브라우저/스크린샷 결과를 UI 구현 계약과 대조하지 않고 완료 판단했는지 여부
...
versionControlContract 없이 Git 작업을 수행했는지 여부
Git 작업이 승인된 includeChanges/excludeChanges, stagingMethod, commitGrouping, pushTarget, historyRewriteOrForcePush와 일치하는지 여부
unrelated dirty work나 사용자 소유 변경을 stage/commit/push에 포함했는지 여부
rebase, amend, force-push, protected branch push, tag 생성, PR 생성이 명시 승인 없이 수행되었는지 여부
bugReportContract 없이 issue 작성, 외부 tracker 등록, bug report 게시를 수행했는지 여부
버그리포트에 재현 절차, 실제 결과, 기대 결과, 환경, 영향도, 증거, redaction 확인이 있는지 여부
버그리포트에서 추정 원인과 확정 사실을 구분했는지 여부
...
UI_IMPLEMENTATION_CONTRACT_MISSING
SCREENSHOT_CONTRACT_COMPARISON_MISSING
...
VERSION_CONTROL_CONTRACT_MISSING
GIT_SCOPE_POLICY_VIOLATION
UNAPPROVED_HISTORY_REWRITE
UNAPPROVED_REMOTE_PUBLISH
BUG_REPORT_CONTRACT_MISSING
BUG_REPORT_REPRODUCTION_MISSING
BUG_REPORT_EVIDENCE_MISSING
BUG_REPORT_REDACTION_MISSING
...
- 웹/모바일 UI 작업의 requiredDocuments에서 `FRONTEND_UX_CRITERIA`, `USER_FLOW` 또는 `INTERACTION_SPEC`, `DESIGN_SYSTEM` 또는 `UI_PATTERN`, `FRONTEND_ARCHITECTURE` 역할 coverage가 확인되는가?
- 프론트엔드 UX 확인이 비어 있는데 route, page, component, layout, styling, motion, visual QA 기준이 구현되지 않았는가?
- `uiImplementationContract`의 레이아웃, 정보 우선순위, 금지 패턴, 반응형 기준, 브라우저/스크린샷 검증 기준이 구현 결과와 일치하는가?
- 현재 사용자 화면 폭에서 먼저 빈 공간, 강조 과다, 정보 우선순위, 숨김 처리, primary/secondary 관계를 확인했는가?
- desktop/mobile 추가 viewport가 필요한 경우 현재 화면 폭 검증 뒤에 확인했는가?
- "DOM에 값이 있다", "접힘이 된다", "테스트가 통과한다"만으로 완료 판단하지 않았는가?
...
## Version Control / Bug Report 검증

다음을 반드시 확인한다.

- Git 작업이 사용자 요청 또는 작업 기준서의 `versionControlContract`에 포함되어 있었는가?
- staged diff, commit diff, pushed branch가 includeChanges 안에 있고 excludeChanges 밖에 있는가?
- unrelated dirty work, 사용자 소유 변경, 이미 staged 된 외부 변경을 임의로 포함하지 않았는가?
- commit grouping과 commit message가 최근 히스토리 스타일과 작업 단위에 맞는가?
- push 대상, upstream, 원격 branch가 계약과 일치하는가?
- rebase, amend, force-push, protected branch push, tag 생성, PR 생성이 명시 승인 없이 수행되지 않았는가?
- Git 작업 후 `git status`, latest commit, remote ref 또는 PR URL처럼 결과 확인 증거가 있는가?
- bug report 작업이 사용자 요청 또는 작업 기준서의 `bugReportContract`에 포함되어 있었는가?
- 버그리포트에 제목, 실제 결과, 기대 결과, 재현 절차, 환경, 영향도, 증거, 이미 시도한 조치, 원하는 후속 조치가 있는가?
- 재현 절차가 독자가 따라 할 수 있을 만큼 구체적인가?
- 추정 원인과 확정 사실을 구분했는가?
- 로그, screenshot, stack trace, failing command, test output을 과도하게 붙여넣지 않고 필요한 증거만 요약했는가?
- 토큰, cookie, credential, 개인정보, 내부 로그 원문, 고객 데이터 같은 민감 정보가 제거되었는가?
...
- testRequirements 충족
- verificationCommands 실행 완료 또는 승인 문서가 허용한 대체 검증 완료
- 작업 기준서 기준 검증 상태가 완료 처리 가능
- Task 범위 초과 없음
...
- 웹/모바일 UI라면 승인 문서 안의 `FRONTEND_UX_CRITERIA`, `USER_FLOW` 또는 `INTERACTION_SPEC`, `DESIGN_SYSTEM` 또는 `UI_PATTERN`, `FRONTEND_ARCHITECTURE` 역할 coverage와 일치
- 웹/모바일 UI라면 브라우저/스크린샷 결과가 `uiImplementationContract`와 일치
...
검증 결과 승인 또는 완료 이동 승인을 요청하기 전에는 `_approval-briefing-language.md`의 "승인 전 브리핑 형식"을 반드시 사용한다. 브리핑은 먼저 확인할 검증/완료 결정과 추천을 제시해야 한다. 승인 문장은 이 승인이 허용하는 완료/수정 판단, 아직 허용하지 않는 작업, 승인하면 고정되는 결정, 남은 위험/중단 조건, 승인 후 실제로 진행할 일을 브리핑한 뒤에만 제시한다.
...
먼저 확인할 결정:
1. 현재 검증 결과를 완료 판단의 근거로 사용할지
   제 추천: 실행한 검증이 작업 범위를 충분히 덮고 남은 위험이 없을 때만 완료 판단으로 이동합니다.
...
승인하면 이렇게 고정됩니다:
1. 검증 결과를 기준으로 완료 보고로 이동할 수 있습니다.

주의할 점:
- 리뷰 중 수정할 점이 있으면 완료로 가지 않고 revision으로 돌아가야 합니다.

승인 후 내가 진행할 일:
- 완료 가능 여부를 확인하고 완료 기록 준비로 이동합니다.
