...
검증 결과 승인 또는 완료 이동 승인을 요청하기 전에는 `_user-facing-language.md`의 "승인 전 브리핑 형식"을 반드시 사용한다. 승인 문장은 검증 목적, 검증된 범위, 검증에서 제외된 범위, 사용자가 승인해야 하는 핵심 결정, 남은 위험/중단 조건, 승인 후 가능해지는 완료 또는 수정 단계를 브리핑한 뒤에만 제시한다.
...
이번 검증은 구현 결과가 승인된 작업 범위와 검증 기준에 맞는지 확인하기 위한 것입니다.
