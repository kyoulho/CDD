...
기존 단일 검증 문서 구조를 따르더라도 기본 읽기 경로의 검증 기록이 400줄 또는 40KB를 넘으면 분리 후보로 보고한다. 1000줄 이상 누적 문서는 active index와 history 문서 분리 후보로 보고한다. 검증 기록은 기본적으로 그 시점의 사실 기록이며 현재 기준 문서가 아니다.

검증 전에는 현재 기준, 과거 기록, 보조 자료를 분리한다. 과거 task, completion, verification, prompt는 historical context로만 보고, active 기준으로 승격된 근거가 없으면 현재 기준으로 사용하지 않는다. generated map, Codesight, agentmemory, search index, recall output, archive branch reference는 보조 자료로만 분류한다.

검증 전에는 현재 작업 포인터와 기본 읽기 경로 계약이 구현 결과 검증에 필요한 최소 문서를 가리키는지 확인한다. 과거 검증 기록이나 완료 기록을 현재 기준처럼 읽어야만 검증할 수 있으면 먼저 문서 정합성 finding으로 보고한다.
...
- 처음에는 "Fast Path", "입력", "최소 guard checklist", "결과 상태"만 먼저 본다.
...
검증 요청이면 먼저 구현 diff 또는 변경 파일, check result, 작업 기준서, `cdd-audit docs --root <project> --format brief --fail-on never`가 가리킨 현재 기준 문서만 읽는다. 과거 verification/completion은 current pointer가 요구하지 않으면 기본으로 열지 않는다.
