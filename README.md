...
버전관리와 버그리포트도 별도 계약이 필요하다. stage/commit/push/rebase/PR 같은 Git 작업은 포함 변경, 제외 변경, 원격 대상, 히스토리 위험을 고정한 `versionControlContract`가 있어야 하고, issue나 bug report는 재현 절차, 실제/기대 결과, 환경, 영향도, 증거, 비밀정보 제거 기준을 고정한 `bugReportContract`가 있어야 한다.
...
- 완료 TASK, old prompt, verification, completion의 rollup/prune 정리 후보
...
```json
{
  ...
  "taskRetention": {
    "activeTaskLimit": 3,
    "recentCompletedTaskLimit": 3,
    "rollupBy": "version-or-milestone"
  },
  "sectionHints": {
...
`taskRetention`은 선택 설정이다. 설정이 없어도 `activeTaskLimit: 3`, `recentCompletedTaskLimit: 3`, `rollupBy: "version-or-milestone"` 기본값으로 warning만 낸다. `TASK 정리 후보`가 나오면 삭제를 자동 실행하지 말고, 현재 기준으로 승격할 결정, 요약으로 압축할 기록, 후속 의존성 때문에 최소 보존할 기록, 삭제 후보, 보류 항목을 사용자에게 브리핑한다.
