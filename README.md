...
반복 읽기를 줄이려면 먼저 `--format brief`를 사용한다. `brief` 출력은 현재 작업 포인터, 먼저 읽을 문서, 읽지 않을 과거 기록/보조 자료, 차단/주의 개수만 보여준다. `brief`는 CDD 판단을 대체하지 않으며, 차단 항목이나 분리 후보의 이유가 필요하면 반드시 text 또는 JSON 출력으로 확장한다.
