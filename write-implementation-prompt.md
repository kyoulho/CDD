...
- 버전관리 확인: Git 작업 전에 포함 변경, 제외 변경, 원격 대상, 히스토리 위험을 고정하는 절차.
- 버그리포트 확인: bug report 작성 전에 재현 절차, 기대/실제 결과, 환경, 증거, 비밀정보 제거를 고정하는 절차.
...
구현 지시서가 stage, commit, push, branch, PR, tag, rebase, amend, force-push를 열려면 버전관리 확인 결론이 `VERSION_CONTROL_ALLOWED`여야 한다. 구현 지시서가 issue 작성, 외부 tracker 등록, bug report 게시를 열려면 버그리포트 확인 결론이 `BUG_REPORT_READY`여야 한다.
...
구현 지시서 요청이면 먼저 `cdd-audit` 실행 경로 규칙에 따라 brief 결과와 승인된 작업 기준서만 읽는다. 사용자가 PATH를 설정했다고 가정하지 말고, PATH 명령이 없으면 CDD skill root의 `bin/cdd-audit` 절대 경로를 시도한다. 이 경로는 시작 조건과 금지 조건을 대체하지 않는다. 시작 조건이 충족되고 새 결정, 정책 충돌, 위험 변경이 없으면 구현 지시서를 내부 실행 기준으로 작성한다. 사용자가 실제 구현까지 명확히 요청했거나 브리핑 승인 후 실행 연계 조건을 만족한 경우에만 같은 요청 범위 안에서 이어간다.
...
- Git 작업이면 `versionControlContract`가 작업 기준서에 포함되어 있다.
- 버그리포트 작업이면 `bugReportContract`가 작업 기준서에 포함되어 있다.
...
- 버전관리 확인이 없거나 `VERSION_CONTROL_BLOCKED`이면 stage, commit, push, branch, PR, tag, rebase, amend, force-push를 포함하는 프롬프트를 만들지 마라.
- 버그리포트 확인이 없거나 `BUG_REPORT_BLOCKED`이면 issue 작성, 외부 tracker 등록, bug report 게시를 포함하는 프롬프트를 만들지 마라.
...
Prompt draft approval은 `PROMPT_DRAFT_APPROVAL`이고, 실제 구현 시작 승인은 `PROMPT_EXECUTION_APPROVAL`이다. 초안 승인만으로 구현을 시작하지 않는다. 다만 구현 지시서 브리핑 후 사용자가 실제 구현까지 승인했고 아래 자동 실행 연계 조건을 모두 만족하면, 구현 지시서 작성 뒤 다시 실행 승인을 요구하지 않고 구현, 검증, 완료 가능 여부 판단까지 이어갈 수 있다.

실제 구현까지 승인한 것으로 보려면 사용자 문장에 실제 구현, 문서 수정, cleanup/delete 실행, revision 실행 중 무엇을 허용하는지 명시되어야 한다. `구현 지시서 초안 작성을 승인합니다`, `브리핑 확인`, `다음 단계 진행`은 실제 실행 승인으로 승격하지 않는다.
...
다만 사용자가 이미 실제 구현, 문서 수정, cleanup/delete 실행, revision 실행까지 명확히 요청했거나 구현 지시서 브리핑 후 실제 실행까지 승인했고, 구현 지시서 작성이 내부 실행 기준을 만드는 절차일 뿐이면, 구현 지시서 작성 뒤 새로 결정할 사항이 없는지 다시 판단한다. 새 제품 판단, 설계 판단, 위험 변경, 범위 확대, 기준 문서 충돌이 없으면 별도의 실행 승인 요청을 반복하지 않고 같은 요청 범위 안에서 `_implementation-rules.md`로 이어간다.

자동 실행 연계 조건:

- 사용자가 실제 실행까지 명확히 요청했거나 구현 지시서 브리핑 후 실제 실행까지 승인했다.
...
- Git 작업이면 포함 변경, 제외 변경, stage 방식, commit grouping, push 대상, history rewrite 위험이 충분하다.
- 버그리포트 작업이면 재현 절차, 기대/실제 결과, 환경, 영향도, 증거, 비밀정보 제거 기준이 충분하다.
...
- 사용자가 초안 검토를 명시했다.
- 사용자가 구현 지시서 초안 작성만 승인했고 실제 실행은 승인하지 않았다.
- 사용자의 승인 문장이 `브리핑 확인`, `초안 작성 승인`, `다음 단계 진행` 수준이고 실제 구현/수정/정리 실행 대상을 명시하지 않았다.
...
사용자가 실제 실행까지 승인했고 새로 결정할 사항이 없습니다.
작업 지시서는 내부 실행 기준으로 작성한 뒤, 같은 요청 범위 안에서 바로 실행합니다.
...
- versionControlContract가 없으면 stage, commit, push, branch, PR, tag, rebase, amend, force-push를 하지 말라는 지시
- bugReportContract가 없으면 issue 작성, 외부 tracker 등록, bug report 게시를 하지 말라는 지시
...
- `versionControlContract`가 없으면 stage, commit, push, branch, PR, tag, rebase, amend, force-push를 만들거나 제안하지 마라.
- Git 작업은 계약의 includeChanges/excludeChanges를 지켜라. unrelated dirty work, 사용자 소유 변경, 이미 staged 된 변경을 넓혀서 포함하지 마라.
- rebase, amend, force-push, protected branch push, tag 생성, PR 생성은 계약에 명시되지 않았으면 하지 마라.
- `bugReportContract`가 없으면 issue 작성, 외부 tracker 등록, bug report 게시를 만들거나 제안하지 마라.
- 버그리포트에는 재현 절차, 실제 결과, 기대 결과, 환경, 영향도, 증거, redaction 확인을 포함하고, 추정 원인과 확정 사실을 구분하라.
...
- 버전관리 작업으로 명시 승인되지 않았으면 커밋하지 마라.
- `versionControlContract` 없이 push, branch, PR, tag, rebase, amend, force-push를 하지 마라.
...
- Prompt Draft Approval과 Prompt Execution Approval을 분리하되, 사용자가 이미 실제 실행까지 명확히 요청했고 브리핑 승인 후 실행 연계 조건을 모두 만족하면 같은 요청 범위 안에서 실행할 수 있다.
...
- bug report 대상 tracker, 제목, 재현 절차, 환경, 영향도, 증거, 비밀정보 제거 기준
- Gradle plugin 추가
