...
구현 지시서 초안, 수정, 실행 승인 요청 전에는 `_user-facing-language.md`의 "승인 전 브리핑 형식"을 반드시 사용한다. 승인 문장은 구현 지시서의 목적, 포함 범위, 제외 범위, 사용자가 승인해야 하는 핵심 결정, 위험/중단 조건, 승인 후 실제로 가능해지는 다음 단계를 브리핑한 뒤에만 제시한다. 브리핑 없이 구현 지시서 승인 문장만 출력하지 않는다.
