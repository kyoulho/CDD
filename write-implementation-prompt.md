...
구현 지시서 요청이면 먼저 `cdd-audit docs --root <project> --format brief --fail-on never` 결과와 승인된 작업 기준서만 읽는다. 이 경로는 시작 조건과 금지 조건을 대체하지 않는다. 시작 조건이 충족되고 새 결정, 정책 충돌, 위험 변경이 없으면 구현 지시서를 내부 실행 기준으로 작성하고, 사용자가 실제 구현까지 요청한 경우 같은 요청 범위 안에서 이어간다.
