...
- `versionControlContract`: Git 작업이 있는 경우 stage, commit, push, branch, PR, tag, rebase, amend, force-push의 포함/제외 범위와 원격 대상, history rewrite 위험을 고정한다.
- `bugReportContract`: bug report 작업이 있는 경우 target tracker, 재현 절차, 실제/기대 결과, 환경, 영향도, 증거, redaction 기준을 고정한다.
...
- `VERSION_CONTROL`이면 `versionControlContract` 없이 Git write 작업을 시작하지 않는다.
- `BUG_REPORT`이면 `bugReportContract` 없이 issue 작성이나 tracker 등록을 시작하지 않는다.
