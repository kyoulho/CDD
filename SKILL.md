---
name: cdd
description: CDD 작업 하네스. 요청이 애매하거나 구현/프론트엔드 UI/UX/인터페이스/DB/API/status/성능/버전관리/버그리포트 작업 전에 제품 기준, 상호작용 방식, 사용자 경험 기준, 저장 의미, 동작 계약, 상태 의미, 운영/품질 기준, Git 작업 계약, 버그리포트 재현 계약, 작업 기준서, 검증 흐름을 확인해야 할 때 사용한다.
---
# CDD Skill
...
- 버전관리 확인: stage, commit, push, branch, rebase, tag, PR 같은 Git 작업 전에 변경 범위, 원격 대상, 히스토리 위험, 사용자 소유 변경을 확인하는 절차.
- 버그리포트 확인: issue/report 작성 전에 대상 저장소, 재현 절차, 기대/실제 결과, 환경, 영향도, 증거, 비밀정보 제거를 확인하는 절차.
...
버전관리나 버그리포트 작업도 구현 세부사항이 아니다. 커밋/푸시/브랜치/리베이스/태그/PR 작업은 `versionControlContract` 없이 진행하지 않고, 버그리포트 작성/등록은 `bugReportContract` 없이 진행하지 않는다.
