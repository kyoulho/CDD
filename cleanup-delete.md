...
- 완료 TASK 산출물 정리는 "TASK 산출물 Rollup / Prune"을 본다.
...
- 완료 TASK, old prompt, verification, completion 전문이 active 문서에 누적되어 rollup/prune 판단이 필요하다.
- 리팩토링이 아니라 폐기가 목적이다.
...
분류 보고에는 각 후보의 이유, 현재 기준과의 충돌 여부, README/index 갱신 필요 여부, `_approval-briefing-language.md`의 "승인 전 브리핑 형식", 사용자 승인 문장을 포함한다. 브리핑은 먼저 확인할 삭제/보존 결정과 추천을 제시해야 한다. 승인 문장은 이 승인이 허용하는 삭제/보존/비-SOT 표시 작업, 아직 허용하지 않는 작업, 되돌리기 어려운 위험, 승인 후 실제로 진행할 정리를 브리핑한 뒤에만 제시한다.
...
## TASK 산출물 Rollup / Prune

완료 TASK 관련 작업 기준서, old prompt, verification, completion 전문은 기본적으로 영구 hot path 보존 대상이 아니다. 정리 요청이나 `cdd-audit`의 `TASK 정리 후보`가 나오면 삭제 여부부터 정하지 말고 먼저 다음으로 분류한다.

- active에 남길 것: 현재 작업, 다음 작업, 최근 완료 요약, 후속 의존성 요약.
- 기준으로 승격할 것: 현재 제품/설계/운영 기준으로 계속 쓰는 결정.
- 요약으로 압축할 것: 완료 사실, 핵심 변경, 검증 결과, 후속 영향만 필요한 기록.
- 최소 보존할 것: migration, 데이터 삭제/보존, public API 제거, 보안/권한 변경, 후속 TASK dependency 근거, 감사 추적.
- 삭제 후보: 현재 기준에 흡수됐고 후속 의존성이나 감사 근거가 없는 전문 기록.
- 보류: 사용자 결정이나 프로젝트 보존 정책이 없어 판단할 수 없는 기록.

정리 브리핑에는 README/index 갱신 필요 여부, 기본 읽기 경로 제외 목록 갱신 필요 여부, 삭제 후보가 실제 삭제인지 history 제외인지, 되돌리기 어려운 위험을 포함한다.

`cdd-audit`의 `TASK 정리 후보`는 삭제 승인도 archive 승인도 아니다. 사용자 승인 전에는 TASK 산출물을 삭제, 이동, archive 처리, 요약으로 대체, 비-SOT 표시, 현재 기준으로 승격하지 않는다.
