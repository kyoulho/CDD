...
- 구현 전 조건은 "시작 조건"을 본다.
- 구현 에이전트의 책임은 "역할"을 본다.
- 실제 구현 중 지켜야 할 규칙은 "구현 규칙"을 본다.
- 성능 위험은 "성능 위험 후보 처리"를 본다.
- 에이전트가 임의로 정하면 안 되는 항목은 "임의 결정 금지 항목"을 본다.
- 테스트 기준이 비어 있을 때는 "테스트 전략이 문서에 없을 때"를 본다.
- 충돌 보고와 완료 보고는 "충돌 발견 시 보고 형식", "완료 보고"를 본다.
...
새 dependency, Gradle plugin, annotation processor, code generation tool, runtime-exposed library는 기본적으로 승인 필요 항목이다. AI는 "단순 편의", "흔한 조합", "표준 라이브러리", "나중에 필요"라는 이유로 dependency를 추가할 수 없다. 승인 문서가 없으면 기존 스택 안에서 구현하는 것이 기본 대안이다. 단, 기존 스택으로 Task 범위와 검증 기준을 충족할 수 없으면 부정확한 우회 구현을 하지 말고 필요한 dependency, 이유, 대안, 영향, 검증 방법을 보고한 뒤 사용자 승인을 요청한다.

Dependency 승인 없이 할 수 없는 작업을 발견하면 다음 순서로 처리한다.

1. 기존 스택으로 정확히 구현 가능한지 확인한다.
2. 가능하면 기존 스택으로 구현하고 dependency를 추가하지 않는다.
3. 불가능하거나 검증 기준을 충족하지 못하면 implementation을 멈춘다.
4. 사용자에게 dependency 이름, 필요 이유, 기존 스택 대안의 한계, 영향 범위, 검증 방법, 추천을 자연어로 보고한다.
5. 사용자가 승인한 뒤 승인 기록이 작업 기준서나 구현 지시서에 남은 경우에만 dependency 변경을 수행한다.
